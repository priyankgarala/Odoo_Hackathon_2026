...
export function VehiclesPage() {
  ...
  const { data: vehicles = [], isLoading } = useQuery({ queryKey: ["vehicles", statusFilter], queryFn: () => getVehicles(statusFilter ? { status: statusFilter } : undefined) });
  ...
  return <div className="space-y-6">
    ...
    {showForm && <Card><CardContent className="pt-6">
      ...
      <form className="grid gap-4 md:grid-cols-2 lg:grid-cols-3" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <Field label="Vehicle type" error={form.formState.errors.type?.message}><Input {...form.register("type", { required: "Required" })} placeholder="Van" /></Field>
        ...
        <Field label="Region"><Input {...form.register("region")} placeholder="West" /></Field>
        {editing && <Field label="Status"><select className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm" {...form.register("status")}>{statuses.map((status) => <option key={status} value={status}>{VEHICLE_STATUS_LABELS[status]}</option>)}</select></Field>}
