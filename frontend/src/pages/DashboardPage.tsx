...
import { REGIONS, VEHICLE_TYPES, type VehicleStatus } from "@/types";
...
export function DashboardPage() {
  ...
  const { data: kpis, isLoading } = useQuery({ queryKey: ["dashboard-kpis", status, region, type], queryFn: () => getDashboardKpis({ status: status || undefined, region: region || undefined, type: type || undefined }) });
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap gap-3 rounded-xl border bg-card p-3"><select className="h-9 rounded-lg border bg-background px-3 text-sm" value={type} onChange={(event) => setType(event.target.value)}><option value="">All vehicle types</option>{VEHICLE_TYPES.map((item) => <option key={item} value={item}>{item}</option>)}</select><select className="h-9 rounded-lg border bg-background px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value as "" | VehicleStatus)}>{statusOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><select className="h-9 rounded-lg border bg-background px-3 text-sm" value={region} onChange={(event) => setRegion(event.target.value)}><option value="">All regions</option>{REGIONS.map((item) => <option key={item} value={item}>{item}</option>)}</select></div>
